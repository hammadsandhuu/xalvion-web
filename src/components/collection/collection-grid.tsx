...
import CollectionGridCard from "@/components/collection/collection-grid-card";
            ...
            <div className="rounded border border-black/5 bg-black/5 w-full overflow-hidden">
                ...
                >
                    {isLoading
                        ...
                        : (categories as Category[])?.slice(0, 8)?.map((category: Category) => (
                            <SwiperSlide key={`category--key-${category.id}`}>
                                <CollectionGridCard
                                    key={category.id}
                                    collection={category}
                                    href={`${ROUTES.CATEGORY}/${category.slug}`}
                                    className="p-4 lg:p-5 bg-white"
                                />
                            </SwiperSlide>
                        ))}
                </Carousel>
